...
if pygame.joystick.get_count() > 0:
    ...
    print("No joystick detected. Running in keyboard-only mode.")
...
# Load PPO model
# This one works sometimes, and slow, use for demonstration learning curve
# ppo_model = PPO.load("./ppo_checkpoints/ppo_checkpoint_1000000")

# This one works well, use for actual sorting, after this training, it overfits
...
def show_hud(current_mode, cam_on, controller_mode=None):
    ...
    cv2.putText(hud, "[M] Manual Mode || B to Exit Manual", (40, 480), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 230, 180), thickness)
    cv2.putText(hud, "[B] Exit Manual", (40, 600), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (200, 200, 255), thickness)
    cv2.putText(hud, "[C] Toggle Camera (On = Slower)", (40, 720), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (180, 230, 255), thickness)
...
def manual_mode():
    ...
    while True:
        ...
        if ((control_mode == "controller") and (joystick_available)) :
            ...
            if x_pressed and not prev_x_pressed:
                ...
                print("Gripper", "Closed" if gripper_state else "Opened")
...
try:
    while True:
        ...
        keys = p.getKeyboardEvents()
        # pygame.event.pump()
        # button_start = joystick.get_button(3)   # Square
        # button_reset = joystick.get_button(1)   # Circle
        # button_manual = joystick.get_button(2)  # Triangle
        # button_camera = joystick.get_button(4)  # L1
        # button_quit = joystick.get_button(8)    # Share


        if ord('s') in keys and keys[ord('s')] & p.KEY_WAS_TRIGGERED:
            print("'s' pressed — start sorting")
            mode = "Sorting"
            show_hud(mode, camera_enabled)
            run_sorting()
            mode = "Idle"
