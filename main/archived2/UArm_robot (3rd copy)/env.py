...
class UArmEnv(gym.Env):
    ...
    def step(self, action=None):
        ...
        dist_to_goal = np.linalg.norm(np.array(box_pos[:2]) - np.array(self.goal_pos_green[:2]))

        reward = -dist_to_goal  # the closer the better

        # Bonus for successful drop
        if not self.picked_up and dist_to_goal < 0.05:
            reward += 10
            done = True
        ...
        if done:
            dist_to_goal = np.linalg.norm(np.array(box_pos[:2]) - np.array(self.goal_pos_green[:2]))
            print(f"✅ Episode done! 📦 Final distance to goal: {dist_to_goal:.4f} m")
