...
class UArmEnv(gym.Env):
    def __init__(self, render=False):
        ...
        self.step_count = 0
        self.MAX_EPISODE_STEPS = 200  # adjust as needed
    ...
    def reset(self):
        ...
        green_box_pos = [2.15, -0.25, 0.025]  # start closer to robot
        ...
        z = 0.25  # fixed height for the end-effector
    ...
    def step(self, action=None):
        ...
        if action is not None:
            # PPO mode
            joint1 = np.clip(action[0], -1.57, 1.57) 
            joint2 = np.clip(action[1], -1.256, 3.14) 
            joint3 = np.clip(action[2], -3.14, 0.38) 
        ...
        reward = -dist_to_goal  # closer = better
        # done = dist_to_goal < 0.02  # if close enough, end episode
        self.step_count += 1

        terminated = dist_to_goal < 0.05
        ...
        if done:
            reason = "🎯 Reached goal!" if terminated else "⏱️ Timeout"
            print(f"{reason} Final distance: {dist_to_goal:.4f} after {self.step_count} steps")
    ...
    # def _get_obs(self):
    #     joint_obs = self.robot.get_joint_obs()
    #     joint_pos = joint_obs['positions'][:3]  # only 3 joints
    #     ee_pos = joint_obs['ee_pos']
    #     box_pos, _ = p.getBasePositionAndOrientation(self.target_box)
    #     obs = np.concatenate([joint_pos, ee_pos, box_pos, self.goal_pos_green])
    #     return obs
    
    def _get_obs(self):
        joint_obs = self.robot.get_joint_obs()
        joint_pos = joint_obs['positions'][:3]
        ee_pos = joint_obs['ee_pos']
        return np.concatenate([joint_pos, ee_pos, self.goal_pos])
