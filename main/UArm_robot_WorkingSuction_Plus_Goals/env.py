...
class UArmEnv(gym.Env):
    def __init__(self, render=False):
        ...
        self.max_steps = 50  # or whatever you want
        self.step_count = 0
    ...
    def step(self, action=None):
        ...
        ee_pos = obs[3:6]
        target_pos = obs[6:9]
        dist_to_goal = np.linalg.norm(ee_pos - target_pos)

        self.step_count += 1
        if (dist_to_goal < 0.05 or self.step_count >= self.max_steps):
            done = True
            print(f"Episode finished after {self.step_count} steps.")
    ...
    def _get_obs(self):
        joint_obs = self.robot.get_joint_obs()
        joint_pos = joint_obs['positions'][:3]       # joints 1–3
        ee_pos = joint_obs['ee_pos']
        target = self.goal_pos                  # or any fixed point
        return np.concatenate([joint_pos, ee_pos, target])
