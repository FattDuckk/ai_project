...
def show_hud(current_mode, cam_on):
    hud = np.zeros((240, 380, 3), dtype=np.uint8)
    ...
    cv2.putText(hud, f"MODE: {current_mode}", (180, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 100), 2)
...
def manual_mode():
    print("🛠️ Entered Manual Mode (press [B] to exit)")
    step_count = 0
    print_interval = 10
    ...
    while True:
        ...
        if step_count % print_interval == 0:
            joint_positions = obs[:3]
            ee_position = obs[3:6]
            print(f"Joint Positions: {np.round(joint_positions, 4)} | End-Effector: {np.round(ee_position, 4)}")
